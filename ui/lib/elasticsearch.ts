...
export async function searchSemantic(
  ...
  const q = geoFilter
    ? {
        bool: {
          must: [{ semantic: { field: "descriptions", query } }],
          filter: [geoFilter],
        },
      }
...
export async function searchBm25(
  ...
  const q = geoFilter
    ? {
        bool: {
          must: [{ match: { descriptions_text: query } }],
          filter: [geoFilter],
        },
      }
...
export async function searchWithReranker(
  ...
  const naive: RankedHotel[] = naiveRes.hits.hits.map(
    (hit: any, i: number) => ({
      ...hitToHotel(hit),
      naiveRank: i + 1,
      rerankedRank: i + 1,
  ...
  const reranked: RankedHotel[] = rerankedRes.hits.hits.map(
    (hit: any, i: number) => {
...
export async function searchByClipVector(
  vector: number[],
  size = 8,
): Promise<Hotel[]> {
  const es = getClient();
...
export async function getClipEmbeddingViaEIS(
  ...
  if (!Array.isArray(vector) || vector.length !== 1024)
    throw new Error(
      `Unexpected EIS CLIP shape: ${JSON.stringify(data).slice(0, 200)}`,
    );
  return vector;
}
...
export async function getOmniEmbeddingViaEIS(input: string): Promise<number[]> {
  ...
  if (!Array.isArray(vector) || vector.length !== 1024)
    throw new Error(
      `Unexpected EIS Omni shape: ${JSON.stringify(data).slice(0, 200)}`,
    );
  return vector;
}
